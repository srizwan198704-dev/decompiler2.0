.class public Lcom/uc/framework/DefaultWindowNew;
.super Lcom/uc/framework/aj;
.source "ProGuard"

# interfaces
.implements Lcom/uc/framework/ui/widget/titlebar/f;
.implements Lcom/uc/framework/ui/widget/toolbar/i;


# instance fields
.field public VQ:Landroid/view/View;

.field private VS:Lcom/uc/framework/e;

.field private VT:Z

.field private VU:Z

.field public VV:Lcom/uc/framework/ui/widget/titlebar/d;

.field public VW:Lcom/uc/framework/ui/widget/toolbar/a;


# direct methods
.method public constructor <init>(Landroid/content/Context;Lcom/uc/framework/e;)V
    .locals 1

    .line 34
    sget v0, Lcom/uc/framework/v;->bJF:I

    invoke-direct {p0, p1, p2, v0}, Lcom/uc/framework/DefaultWindowNew;-><init>(Landroid/content/Context;Lcom/uc/framework/e;I)V

    return-void
.end method

.method public constructor <init>(Landroid/content/Context;Lcom/uc/framework/e;I)V
    .locals 0

    .line 38
    invoke-direct {p0, p1, p2, p3}, Lcom/uc/framework/aj;-><init>(Landroid/content/Context;Lcom/uc/framework/o;I)V

    const/4 p1, 0x0

    .line 30
    iput-boolean p1, p0, Lcom/uc/framework/DefaultWindowNew;->VT:Z

    const/4 p1, 0x1

    .line 31
    iput-boolean p1, p0, Lcom/uc/framework/DefaultWindowNew;->VU:Z

    .line 39
    iput-object p2, p0, Lcom/uc/framework/DefaultWindowNew;->VS:Lcom/uc/framework/e;

    .line 41
    invoke-virtual {p0}, Lcom/uc/framework/DefaultWindowNew;->kz()Lcom/uc/framework/ui/widget/titlebar/d;

    move-result-object p1

    iput-object p1, p0, Lcom/uc/framework/DefaultWindowNew;->VV:Lcom/uc/framework/ui/widget/titlebar/d;

    .line 42
    invoke-virtual {p0}, Lcom/uc/framework/DefaultWindowNew;->kA()Lcom/uc/framework/ui/widget/toolbar/a;

    move-result-object p1

    iput-object p1, p0, Lcom/uc/framework/DefaultWindowNew;->VW:Lcom/uc/framework/ui/widget/toolbar/a;

    .line 43
    invoke-virtual {p0}, Lcom/uc/framework/DefaultWindowNew;->km()Landroid/view/View;

    move-result-object p1

    iput-object p1, p0, Lcom/uc/framework/DefaultWindowNew;->VQ:Landroid/view/View;

    return-void
.end method

.method public static kp()Landroid/widget/RelativeLayout$LayoutParams;
    .locals 3

    .line 130
    new-instance v0, Landroid/widget/RelativeLayout$LayoutParams;

    const v1, 0x7f05166c

    .line 131
    invoke-static {v1}, Lcom/uc/framework/resources/v;->getDimension(I)F

    move-result v1

    float-to-int v1, v1

    const/4 v2, -0x1

    invoke-direct {v0, v2, v1}, Landroid/widget/RelativeLayout$LayoutParams;-><init>(II)V

    const/16 v1, 0xc

    .line 132
    invoke-virtual {v0, v1}, Landroid/widget/RelativeLayout$LayoutParams;->addRule(I)V

    return-object v0
.end method

.method public static kq()Lcom/uc/framework/k;
    .locals 2

    .line 157
    new-instance v0, Lcom/uc/framework/k;

    const v1, 0x7f051649

    .line 158
    invoke-static {v1}, Lcom/uc/framework/resources/v;->getDimension(I)F

    move-result v1

    float-to-int v1, v1

    invoke-direct {v0, v1}, Lcom/uc/framework/k;-><init>(I)V

    const/4 v1, 0x2

    .line 159
    iput v1, v0, Lcom/uc/framework/k;->type:I

    return-object v0
.end method

.method public static kr()Lcom/uc/framework/k;
    .locals 2

    .line 164
    new-instance v0, Lcom/uc/framework/k;

    const v1, 0x7f05166c

    .line 165
    invoke-static {v1}, Lcom/uc/framework/resources/v;->getDimension(I)F

    move-result v1

    float-to-int v1, v1

    invoke-direct {v0, v1}, Lcom/uc/framework/k;-><init>(I)V

    const/4 v1, 0x3

    .line 166
    iput v1, v0, Lcom/uc/framework/k;->type:I

    return-object v0
.end method


# virtual methods
.method public final a(Lcom/uc/framework/ui/widget/toolbar/f;)V
    .locals 0

    return-void
.end method

.method public b(Lcom/uc/framework/ui/widget/toolbar/f;)V
    .locals 1

    .line 3253
    iget p1, p1, Lcom/uc/framework/ui/widget/toolbar/f;->mId:I

    const v0, 0x7ffe2001

    if-eq p1, v0, :cond_0

    goto :goto_0

    .line 210
    :cond_0
    iget-object p1, p0, Lcom/uc/framework/DefaultWindowNew;->VS:Lcom/uc/framework/e;

    const/4 v0, 0x1

    invoke-interface {p1, v0}, Lcom/uc/framework/e;->onWindowExitEvent(Z)V

    :goto_0
    return-void
.end method

.method public bx(I)V
    .locals 0

    return-void
.end method

.method public kA()Lcom/uc/framework/ui/widget/toolbar/a;
    .locals 3

    .line 100
    new-instance v0, Lcom/uc/framework/ui/widget/toolbar/a;

    invoke-virtual {p0}, Lcom/uc/framework/DefaultWindowNew;->getContext()Landroid/content/Context;

    move-result-object v1

    invoke-direct {v0, v1}, Lcom/uc/framework/ui/widget/toolbar/a;-><init>(Landroid/content/Context;)V

    .line 101
    invoke-virtual {v0, p0}, Lcom/uc/framework/ui/widget/toolbar/a;->a(Lcom/uc/framework/ui/widget/toolbar/i;)V

    const/16 v1, 0x1001

    .line 102
    invoke-virtual {v0, v1}, Lcom/uc/framework/ui/widget/toolbar/a;->setId(I)V

    .line 103
    invoke-virtual {p0}, Lcom/uc/framework/DefaultWindowNew;->EI()I

    move-result v1

    sget v2, Lcom/uc/framework/v;->bJF:I

    if-ne v1, v2, :cond_0

    .line 3195
    iget-object v1, p0, Lcom/uc/framework/aj;->awD:Landroid/view/ViewGroup;

    .line 104
    invoke-static {}, Lcom/uc/framework/DefaultWindowNew;->kr()Lcom/uc/framework/k;

    move-result-object v2

    invoke-virtual {v1, v0, v2}, Landroid/view/ViewGroup;->addView(Landroid/view/View;Landroid/view/ViewGroup$LayoutParams;)V

    goto :goto_0

    .line 3207
    :cond_0
    iget-object v1, p0, Lcom/uc/framework/aj;->bKy:Landroid/widget/RelativeLayout;

    .line 106
    invoke-static {}, Lcom/uc/framework/DefaultWindowNew;->kp()Landroid/widget/RelativeLayout$LayoutParams;

    move-result-object v2

    invoke-virtual {v1, v0, v2}, Landroid/widget/RelativeLayout;->addView(Landroid/view/View;Landroid/view/ViewGroup$LayoutParams;)V

    :goto_0
    return-object v0
.end method

.method public final kk()Lcom/uc/framework/ui/widget/titlebar/d;
    .locals 1

    .line 55
    iget-object v0, p0, Lcom/uc/framework/DefaultWindowNew;->VV:Lcom/uc/framework/ui/widget/titlebar/d;

    if-eqz v0, :cond_0

    iget-object v0, p0, Lcom/uc/framework/DefaultWindowNew;->VV:Lcom/uc/framework/ui/widget/titlebar/d;

    instance-of v0, v0, Lcom/uc/framework/ui/widget/titlebar/d;

    if-eqz v0, :cond_0

    .line 56
    iget-object v0, p0, Lcom/uc/framework/DefaultWindowNew;->VV:Lcom/uc/framework/ui/widget/titlebar/d;

    goto :goto_0

    :cond_0
    const/4 v0, 0x0

    :goto_0
    return-object v0
.end method

.method public km()Landroid/view/View;
    .locals 3

    .line 70
    new-instance v0, Landroid/view/View;

    invoke-virtual {p0}, Lcom/uc/framework/DefaultWindowNew;->getContext()Landroid/content/Context;

    move-result-object v1

    invoke-direct {v0, v1}, Landroid/view/View;-><init>(Landroid/content/Context;)V

    const/high16 v1, -0x10000

    .line 71
    invoke-virtual {v0, v1}, Landroid/view/View;->setBackgroundColor(I)V

    .line 2195
    iget-object v1, p0, Lcom/uc/framework/aj;->awD:Landroid/view/ViewGroup;

    .line 72
    invoke-virtual {p0}, Lcom/uc/framework/DefaultWindowNew;->ks()Lcom/uc/framework/k;

    move-result-object v2

    invoke-virtual {v1, v0, v2}, Landroid/view/ViewGroup;->addView(Landroid/view/View;Landroid/view/ViewGroup$LayoutParams;)V

    return-object v0
.end method

.method public ks()Lcom/uc/framework/k;
    .locals 3

    .line 171
    new-instance v0, Lcom/uc/framework/k;

    const/4 v1, -0x1

    invoke-direct {v0, v1}, Lcom/uc/framework/k;-><init>(I)V

    const/4 v1, 0x1

    .line 174
    iput v1, v0, Lcom/uc/framework/k;->type:I

    .line 176
    sget v1, Lcom/uc/framework/v;->bJF:I

    invoke-virtual {p0}, Lcom/uc/framework/DefaultWindowNew;->EI()I

    move-result v2

    if-eq v1, v2, :cond_1

    .line 177
    iget-object v1, p0, Lcom/uc/framework/DefaultWindowNew;->VV:Lcom/uc/framework/ui/widget/titlebar/d;

    if-eqz v1, :cond_0

    const v1, 0x7f051649

    .line 178
    invoke-static {v1}, Lcom/uc/framework/resources/v;->getDimension(I)F

    move-result v1

    float-to-int v1, v1

    iput v1, v0, Lcom/uc/framework/k;->topMargin:I

    .line 181
    :cond_0
    iget-object v1, p0, Lcom/uc/framework/DefaultWindowNew;->VW:Lcom/uc/framework/ui/widget/toolbar/a;

    if-eqz v1, :cond_1

    const v1, 0x7f05166c

    .line 182
    invoke-static {v1}, Lcom/uc/framework/resources/v;->getDimension(I)F

    move-result v1

    float-to-int v1, v1

    iput v1, v0, Lcom/uc/framework/k;->bottomMargin:I

    :cond_1
    return-object v0
.end method

.method public final kt()V
    .locals 0

    return-void
.end method

.method public final ku()V
    .locals 1

    .line 228
    iget-boolean v0, p0, Lcom/uc/framework/DefaultWindowNew;->VT:Z

    if-eqz v0, :cond_0

    return-void

    :cond_0
    const/4 v0, 0x1

    .line 230
    iput-boolean v0, p0, Lcom/uc/framework/DefaultWindowNew;->VT:Z

    .line 231
    invoke-virtual {p0}, Lcom/uc/framework/DefaultWindowNew;->vR()Z

    move-result v0

    iput-boolean v0, p0, Lcom/uc/framework/DefaultWindowNew;->VU:Z

    const/4 v0, 0x0

    .line 232
    invoke-virtual {p0, v0}, Lcom/uc/framework/DefaultWindowNew;->bW(Z)V

    .line 233
    invoke-virtual {p0}, Lcom/uc/framework/DefaultWindowNew;->kk()Lcom/uc/framework/ui/widget/titlebar/d;

    move-result-object v0

    if-eqz v0, :cond_1

    .line 234
    invoke-virtual {p0}, Lcom/uc/framework/DefaultWindowNew;->kk()Lcom/uc/framework/ui/widget/titlebar/d;

    move-result-object v0

    invoke-interface {v0}, Lcom/uc/framework/ui/widget/titlebar/d;->ku()V

    .line 237
    :cond_1
    invoke-virtual {p0}, Lcom/uc/framework/DefaultWindowNew;->kv()V

    return-void
.end method

.method public kv()V
    .locals 0

    return-void
.end method

.method public final kw()V
    .locals 1

    .line 245
    iget-boolean v0, p0, Lcom/uc/framework/DefaultWindowNew;->VT:Z

    if-nez v0, :cond_0

    return-void

    :cond_0
    const/4 v0, 0x0

    .line 247
    iput-boolean v0, p0, Lcom/uc/framework/DefaultWindowNew;->VT:Z

    .line 248
    iget-boolean v0, p0, Lcom/uc/framework/DefaultWindowNew;->VU:Z

    invoke-virtual {p0, v0}, Lcom/uc/framework/DefaultWindowNew;->bW(Z)V

    .line 249
    invoke-virtual {p0}, Lcom/uc/framework/DefaultWindowNew;->kk()Lcom/uc/framework/ui/widget/titlebar/d;

    move-result-object v0

    if-eqz v0, :cond_1

    .line 250
    invoke-virtual {p0}, Lcom/uc/framework/DefaultWindowNew;->kk()Lcom/uc/framework/ui/widget/titlebar/d;

    move-result-object v0

    invoke-interface {v0}, Lcom/uc/framework/ui/widget/titlebar/d;->mK()V

    .line 252
    :cond_1
    invoke-virtual {p0}, Lcom/uc/framework/DefaultWindowNew;->kx()V

    return-void
.end method

.method public kx()V
    .locals 0

    return-void
.end method

.method public ky()V
    .locals 1

    .line 261
    iget-object v0, p0, Lcom/uc/framework/DefaultWindowNew;->VS:Lcom/uc/framework/e;

    invoke-interface {v0}, Lcom/uc/framework/e;->onTitleBarBackClicked()V

    return-void
.end method

.method public kz()Lcom/uc/framework/ui/widget/titlebar/d;
    .locals 2

    .line 62
    new-instance v0, Lcom/uc/framework/ui/widget/titlebar/h;

    invoke-virtual {p0}, Lcom/uc/framework/DefaultWindowNew;->getContext()Landroid/content/Context;

    move-result-object v1

    invoke-direct {v0, v1, p0}, Lcom/uc/framework/ui/widget/titlebar/h;-><init>(Landroid/content/Context;Lcom/uc/framework/ui/widget/titlebar/f;)V

    .line 63
    invoke-static {}, Lcom/uc/framework/DefaultWindowNew;->kq()Lcom/uc/framework/k;

    move-result-object v1

    invoke-virtual {v0, v1}, Lcom/uc/framework/ui/widget/titlebar/h;->setLayoutParams(Landroid/view/ViewGroup$LayoutParams;)V

    const/16 v1, 0x1000

    .line 64
    invoke-virtual {v0, v1}, Lcom/uc/framework/ui/widget/titlebar/h;->setId(I)V

    .line 1195
    iget-object v1, p0, Lcom/uc/framework/aj;->awD:Landroid/view/ViewGroup;

    .line 65
    invoke-virtual {v1, v0}, Landroid/view/ViewGroup;->addView(Landroid/view/View;)V

    return-object v0
.end method

.method public onThemeChange()V
    .locals 1

    .line 48
    invoke-virtual {p0}, Lcom/uc/framework/DefaultWindowNew;->kk()Lcom/uc/framework/ui/widget/titlebar/d;

    move-result-object v0

    if-eqz v0, :cond_0

    .line 49
    invoke-virtual {p0}, Lcom/uc/framework/DefaultWindowNew;->kk()Lcom/uc/framework/ui/widget/titlebar/d;

    move-result-object v0

    invoke-interface {v0}, Lcom/uc/framework/ui/widget/titlebar/d;->onThemeChange()V

    :cond_0
    return-void
.end method

.method public final setTitle(Ljava/lang/String;)V
    .locals 1

    .line 77
    invoke-virtual {p0}, Lcom/uc/framework/DefaultWindowNew;->kk()Lcom/uc/framework/ui/widget/titlebar/d;

    move-result-object v0

    if-eqz v0, :cond_0

    .line 78
    invoke-virtual {p0}, Lcom/uc/framework/DefaultWindowNew;->kk()Lcom/uc/framework/ui/widget/titlebar/d;

    move-result-object v0

    invoke-interface {v0, p1}, Lcom/uc/framework/ui/widget/titlebar/d;->setTitle(Ljava/lang/String;)V

    :cond_0
    return-void
.end method
